                    ...
                    <div class="space-y-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                        ...
                        @if ($abertos->isEmpty())
                            <div class="rounded-xl border border-slate-200 bg-white p-6 text-sm text-slate-600">
                                <p>Nenhum edital aberto encontrado.</p>
                                <div class="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-4">
                                    <p class="text-sm font-semibold text-slate-900">
                                        {{ $proximos->isEmpty() ? 'Ainda não temos próximos editais publicados.' : 'Enquanto isso, você já pode acompanhar as próximas oportunidades.' }}
                                    </p>
                                    <p class="mt-1 text-sm text-slate-600">Para receber avisos sobre novas oportunidades, assine nossa newsletter.</p>
                                    <button type="button" class="mt-3 inline-flex items-center rounded-md border border-blue-300 bg-blue-100 px-4 py-2 text-sm font-semibold text-blue-800 hover:bg-blue-200" @click="newsletterOpen = true">
                                    ...
                                    <article class="rounded-xl border border-emerald-200 bg-emerald-50/60 p-4 shadow-sm">
                                        <div class="flex flex-wrap items-start justify-between gap-3">
                                            <div class="min-w-0 flex-1">
                                                ...
                                                <p class="mt-1 text-sm leading-6 text-slate-600">{{ $edital->descricao ?: 'Sem descrição.' }}</p>
                                        ...
                                        <div class="mt-3 flex flex-wrap gap-2">
                                            ...
                                            @if ($edital->hasArquivoEdital())
                                                <a href="{{ route('public.editais.download', $edital) }}" class="inline-flex items-center rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Baixar Edital</a>
                                            @endif
                                        </div>
                                    </article>
                                @endforeach
                                ...
                                <div class="space-y-3">
                                    @foreach ($consultaResultados as $item)
                                        <article class="rounded-lg border border-slate-200 p-4 text-sm" x-data="{ openInfoModal: false, openEditModal: false, openResendModal: false }">
                                            ...
                                            @endif
                                            @if (!empty($editLinkError) && (int) $editLinkTargetId === (int) ($item['id'] ?? 0))
                                                <div class="mt-3 rounded-lg border border-amber-300 bg-amber-50 p-3">
                                                    <span class="inline-flex items-center rounded-full border border-amber-400 bg-amber-100 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-amber-800">
                                                        Atenção
                                                    </span>
                                                    <p class="mt-1 text-xs font-semibold text-amber-800">{{ $editLinkError }}</p>
                                                </div>
                                            @endif
                                            @if (empty($item['email_verificado']) && !empty($item['id']))
                                                <p class="mt-3 text-xs text-amber-700">Sem verificação, a candidatura pode ser indeferida automaticamente.</p>
                                            @endif
                                        </article>
                                    @endforeach
                                </div>
